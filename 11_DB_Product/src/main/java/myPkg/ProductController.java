...
public class ProductController extends HttpServlet {
	...
	private void doAction(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		// insert.prd 요청은 insertForm.jsp에서 submit을 클릭했을 때 요청할 수 있고,
		// list.jsp에서 새로고침 했을 때 insert.prd를 요청할 수 있다.
		if(command.equals("/insert.prd")) {
			System.out.println("insert.prd 요청");
			
			System.out.println("flag:" + application.getAttribute("flag"));
			if(application.getAttribute("flag").equals("false")) { // insertForm.jsp submit클릭해서 
		...
		// update2
			...
			// 입력3(name,price,company + hidden1(id)
