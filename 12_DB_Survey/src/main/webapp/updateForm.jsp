...
<%
	...
	// 자바 배열을 JSP 내장 객체(pageContext)의 속성(attribute)으로 저장합니다.
	// EL에서 ${?} 로 접근 가능
	pageContext.setAttribute("part",part);
	pageContext.setAttribute("howto", howto);
%>
...
<form action="update.sv" method="post">
	...
	<c:forEach var="item" items="<%=satisfaction%>">
        <input type="radio" name = "satisfaction" value="${item}"
        <c:if test="${sb.satisfaction == item}"> checked="checked"</c:if>
        >${item}
    </c:forEach>
	<p>
	...
	<c:forEach var="item" items="<%=part%>">
        <input type="checkbox" name = "part" value="${item}"
            <c:if test="${fn:contains(sb.part, item)}">
                checked="checked"
            </c:if>
        >${item}
    </c:forEach>
	...
	정보 발송 방법 
		<select name="howto">
		    <c:forEach var="h" items="${howto}">
		        <option value="${h}" 
		            <c:if test="${sb.howto eq h}">selected</c:if>>${h}
		        </option>
		    </c:forEach>
		</select>
