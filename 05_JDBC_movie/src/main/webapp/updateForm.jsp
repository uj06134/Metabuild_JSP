<%@ page language="java" contentType="text/html; charset=UTF-8"
	pageEncoding="UTF-8"%>
<%@ page import = "myPkg.*" %>

<jsp:useBean id="mdao" class="myPkg.MovieDao"/>
<%
	String num = request.getParameter("num");
	MovieBean mb = mdao.getMovieByNum(num);
%>

<% 
	String [] genre = {"공포", "다큐", "액션", "애니메이션"};
	String [] time = {"08~10","10~12","12~14","14~16","16~18"};
%>

	<h2>영화 선호도 조사 수정 </h2>
	<form action="updateProc.jsp" method="post">
	
		<!-- form안에 input이 있어야 받은 num이 넘어감 -->
		<input type="hidden" name="num" value="<%=num %>">
		<table border="1">
		<tr>
			<td>아이디</td>
			<td><input type="text" name="id" value="<%= mb.getId() %>"></td>  
		</tr>
		<tr>
			<td>이름</td><td> 
			<input type="text" name="name" value="<%= mb.getName() %>"></td> 
		</tr> 
		<tr>
			<td>나이</td>
			<td><input type="text" name="age" value="<%= mb.getAge() %>"></td> 
		</tr>
		<tr>
			<td>좋아하는 장르</td>
			<td>
			<%for(String g : genre) { %>
            	<input type="checkbox" name="genre" value="<%= g %>" <%if(mb.getGenre().contains(g)){ %> checked <%}%> ><%= g %>         
			<%} %>
				
			</td> 
		</tr>

		<tr>
			<td>즐겨보는 시간대</td>
			<td>
		
				<select name="time">
					<%for(int i=0; i<time.length; i++){ %>
					    <option value="<%=time[i] %>" <%if(mb.getTime().equals(time[i])){%> selected <%} %>><%=time[i] %></option>

					<%} %>
				</select>
		<tr>
			<td>동반 관객수</td>
			<td>
				<%
				for(int i=1;i<=5;i++){
				%>
					<input type="radio" name="partner" value="<%=i%>" <%if(mb.getPartner()==i){ %> checked <%}%>><%=i%>
				<%} %>
			</td>
		</tr>
		
		<tr>
			<td>영화관 개선사항</td>
			<td>
				<textarea name="memo" cols="40" rows="3"><%= mb.getMemo() %></textarea> 
			</td>
		</tr>
		<tr align="center">
			<td colspan="2">
				<input type="submit" value="수정하기">
			</td>
		</tr>
		</table>
	</form>

</html>

